  ...
  */
  if (success) {
	...
	carry = 0.0;
      ...
      */
      /*
	so we need to have negmask realy be a double
	we need i8_twm24 to be and int64_t that overlaps
      */
      int64_t neg_mask;
      int64_t zero_l;
      int64_t *i8_twom24_p;
      ...
      i8_twom24_p = (int64_t*)&twom24;
      i8_twom48_p = (int64_t*)&twom48;
      i8_carry_p  = (int64_t*)&carry;
      i8_epsilon_p = (int64_t*)&epsilon;
      i8_one_l_p   = (int64_t*)&one_l;

      neg_mask       = zero_l - (int64_t)(uni < 0.0);
      *i8_carry_p    = neg_mask & (*i8_twom24_p);
      *i8_sigma_p    = neg_mask & (*i8_onel_p);
      uni            = uni + sigma;
      seeds[i24]     = uni;
      i24            = inext[i24];
      j24            = jnext[j24];
	...
	small numbers (with less than 12 "significant" bits) are "padded".
      */
      small_mask    = zero_l - (int64_t)(uni < twom12);
      *i8_epsilon_p = small_mask & (*i8_twom24_p);
      uni += epsilon * seeds[j24];
      /*
	And sero is forbidden in case some takes a log.
      */
      zero_mask = zero_l - (int64_t)(uni == 0.0);
      *i8_mu_p  = zero_mask & (*i8_twom48_p);
      uni += mu;
      recv[ivec] = uni;
