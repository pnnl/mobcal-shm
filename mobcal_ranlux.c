...
int mobcal_ranlux(struct mobcal_state_struct *state,
  ...
  */
  ...
  if (success) {
    if (rstate->uninitialized) {
      ...
      k2  = 0;
      /*
	Finish initializing ranlux state and set
	uninitialized field to 0.
      */
      success = mobcal_rluxgo(state,lux,jseed,k1,k2);
    }
  }
  /*
C
  ...
  if (success) {
	...
	*/
    ...
    /*
      ...
      added a 1 in the inside increment of kount.
    kount = kount + lenv;
    if (kount >= igiga) {
      mkount = mkount + 1;
      kount = kount -igiga;
